	...
	return (
		<div className="block-add-comment__relationship-btns">
			...
			<div className="block-add-comment__list">
				{statusNewComment.map(configBtn => <button onClick={() => onUpdate(configBtn)} title={`При отметке другие пользователи смогут "явно" видеть посыл комментария`} key={configBtn._id} className={`block-add-comment__btn-status block-add-comment__btn-status_${configBtn.value}` + (currentStatus === configBtn.value ? " active" : "")} type="button">{configBtn.text}</button>)}
			</div>
